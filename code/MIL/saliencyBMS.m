...
function [saliencyRegions] = saliencyBMS(curImg, cur_filename, Dir, regionNum, maxInd)
...
%     addpath(genpath('Saliency\BMS-mex'));
    ...
    imwrite(curImg,[input_dir filesep 'curImg' num2str(maxInd) '.jpg']);
    ...
    movefile([output_dir filesep 'curImg' num2str(maxInd) '.png'],[output_dir filesep 'curImg-eye' num2str(maxInd) '.png']);
    I_eye               =   imread([output_dir filesep 'curImg-eye' num2str(maxInd) '.png']);
...
%     mexBMSSalient(input_dir,output_dir,sample_step_size,opening_width,dilation_width_1,dilation_width_2,blur_std,use_normalization,handle_border);
    ...
    imwrite(I,fullfile(output_dir,['curImg' num2str(maxInd) '.png']));
    
    saliencyRegions = extractRegions(curImg, cur_filename, I_eye, I, Dir, regionNum);
       
    delete([input_dir filesep 'curImg' num2str(maxInd) '.jpg']);
    delete([output_dir filesep 'curImg' num2str(maxInd) '.png']);
end
...
function [curRegions] = extractRegions(curImg, cur_filename, I_eye, I, Dir, regionNum)
    if exist([Dir filesep 'regionCoordsSaliency-' num2str(regionNum) '.mat'],'file')
        load([Dir filesep 'regionCoordsSaliency-' num2str(regionNum) '.mat']);
        curInd = find(strcmp({coord.filename}, cur_filename)==1);
        if ~isempty(curInd)
            parfor index = 1:size(coord(curInd).bb,1)
                curRegions{index} = imcrop(curImg,coord(curInd).bb(index,:));
            end
            return;
        else
    ...
    eyeThr              =   [100 95  90   85   80  75  70   65   60  55  50   45   40   35   30   25   20   15   10  ];  
    saliencyThr         =   [0.4 0.4 0.35 0.35 0.3 0.3 0.25 0.25 0.2 0.2 0.15 0.15 0.10 0.10 0.05 0.05 0.05 0.02 0.02]; 
    bbRatio             =   1/3;
    bbRatio2            =   1/2;
    sizeThr             =   15;
...
%         rectangle('Position',curBBExpanded(index,:),'EdgeColor','r');
...
%     rectangle('Position',mainbb,'EdgeColor','g');
    curRegions{index+1} = imcrop(curImg,mainbb);
%     hold off

    coord(curInd).filename = cur_filename;
    coord(curInd).bb = [curBBExpanded;mainbb];
    save([Dir filesep 'regionCoordsSaliency-' num2str(regionNum) '.mat'],'coord');
end

function [regions, input_morph] = refineRegions(input)
    regionSizeThr       =   [2*min(size(input))/3 100000];
    radius              =   3;
