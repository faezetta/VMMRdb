...
-- ADDED
function ImagenetDataset:get_instances(i)
   local path = ffi.string(self.imageInfo.imagePath[i]:data())
   local prefix = '/home/keishin/sandbox/CARS/mil/Merged_MILCNN'
   local class_name, file_name, ext = path:match("([^//]+)/([^//]+)(.jpg)")

   local dir_files = paths.concat(prefix, self.split, class_name, file_name)
   local files ={}
   for f in paths.files(dir_files) do
      if 2<#f  then
         table.insert(files, paths.concat(dir_files, f))
      end
   end
   assert(#files ~= 0,'directory has no file' .. dir_files)
   local bag = torch.FloatTensor(#files, 3, 224, 224)
   for j=1, #files do
      bag[j] = self:_loadImage(files[j])
   end
   local class = self.imageInfo.imageClass[i]
   return {
      input = bag,
